...
class NumberbaseconverterAPIClientError(Exception):
...
class ValidationError(NumberbaseconverterAPIClientError):
...
class NumberbaseconverterAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"value": {"type": "string", "required": True}, "from": {"type": "integer", "required": False, "min": 2, "max": 36, "default": 10}, "to": {"type": "integer", "required": False, "min": 2, "max": 36}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the NumberbaseconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises NumberbaseconverterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/numberbaseconverter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise NumberbaseconverterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[NumberbaseconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns NumberBaseConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises NumberbaseconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise NumberbaseconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise NumberbaseconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise NumberbaseconverterAPIClientError(f"Request failed: {str(e)}")
